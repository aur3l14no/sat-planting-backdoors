...
void printGraph()
{
    ...
    for (int i = 1; i <= n; i++)
        if (log(radius[0][i]) < minRadVar)
            minRadVar = log(radius[0][i]);
    for (int j = 1; j <= m; j++)
        if (log(radius[1][j]) < minRadClau)
            minRadClau = log(radius[1][j]);
    double size = 1;
...
int generateFormula(int n, int m, int k, int K, double b, double B, double T, bool varRename, int seed)
{
    n = n;
    m = m;
    k = k;
...
PYBIND11_MODULE(popularity_similarity, m)
{
    m.doc() = "SAT formula generator based on popularity similarity model.";
    m.def("generate", &generateFormula, "Generate formula",
          "n"_a = 1000,          // Number of nodes
          "m"_a = 3700,          // Number of clauses
          "k"_a = 0,             // Average size of clauses (flexible part)
          "K"_a = 3,             // Rigid clause size
          "b"_a = 0.83,          // Beta for vars
          "B"_a = 1.00,          // Beta for clauses
          "T"_a = 1.9,           // Temperature
          "varRename"_a = false, // if true varRename variables acording to angle
          "seed"_a = 0           // Random seed
    );
}
